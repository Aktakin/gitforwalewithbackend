...
-- Policy: Anyone can read public skills
CREATE POLICY "Anyone can read public skills" ON public.skills
...
-- Policy: Anyone can read public requests
CREATE POLICY "Anyone can read public requests" ON public.requests
  FOR SELECT USING (is_public = true OR auth.uid() = user_id);
...
-- Policy: Users can read proposals for their requests
...
-- ============================================
-- 5. CONVERSATIONS TABLE (for messages)
-- ============================================
...
-- Policy: Users can update their own messages (mark as read, edit, etc.)
...
-- Note: In production, you might want to restrict this to service role
...
-- 1. Table Editor → should see all tables
-- 2. Authentication → Settings → confirm email settings
-- ============================================
